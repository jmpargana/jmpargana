...
type RecursiveRequired<T extends Record<string, any>> = {
    [P in keyof T]-?: T[P] extends Record<string, any> | Array<any> ? RecursiveRequired<T[P]> : T[P];
};
type ObjectSetter<Data extends Obj, Path extends string = string> = (<P extends Path, V extends Traverse<Data, P> = Traverse<Data, P>>(path: P, value: V) => void) & (<P extends Path, V extends Traverse<Data, P> = Traverse<Data, P>>(path: P, updater: (value: V) => V) => void) & ((value: Data) => void) & ((updater: (value: Data) => Data) => void);
type UnknownObjectSetter<Data extends Obj, Path extends string = string> = ((path: string, value: unknown) => void) & (<P extends Path, V extends Traverse<Data, P> = Traverse<Data, P>>(path: P, updater: (value: V) => unknown) => void) & ((updater: (value: Data) => unknown) => void) & ((value: unknown) => void);
type PartialErrorsSetter<Data extends Obj, Path extends string = string> = (<P extends Path, V extends Traverse<AssignableErrors<Data>, P> = Traverse<AssignableErrors<Data>, P>>(path: P, value: V) => void) & (<P extends Path, V extends Traverse<AssignableErrors<Data>, P> = Traverse<AssignableErrors<Data>, P>>(path: P, updater: (value: V) => V) => void) & ((value: AssignableErrors<Data>) => void) & ((updater: (value: Errors<Data>) => AssignableErrors<Data>) => void);
type PrimitiveSetter<Data> = ((value: Data) => void) & ((updater: (value: Data) => Data) => void);
type FieldsSetter<Data extends Obj, Path extends string = string> = (<P extends Path, V extends Traverse<Data, P> = Traverse<Data, P>>(path: P, value: V, shouldTouch?: boolean) => void) & (<P extends Path, V extends Traverse<Data, P> = Traverse<Data, P>>(path: P, updater: (value: V) => V, shouldTouch?: boolean) => void) & ((value: Data) => void) & ((updater: (value: Data) => Data) => void);
type UnknownFieldsSetter<Data extends Obj, Path extends string = string> = (<P extends Path>(path: P, value: unknown, shouldTouch?: boolean) => void) & (<P extends Path, V extends Traverse<Data, P> = Traverse<Data, P>>(path: P, updater: (value: V) => unknown, shouldTouch?: boolean) => void) & ((value: unknown) => void) & ((updater: (value: Data) => unknown) => void);
type Setter<Data, Path extends string = string> = Data extends Record<string, any> ? ObjectSetter<Data, Path> : PrimitiveSetter<Data>;
type DeepSetResult<Data extends Obj | Obj[], Value> = {
...
type KnownHelpers<Data extends Obj, Path extends string = string> = Omit<Helpers<Data, Path>, "setData" | "setFields"> & {
    setData: ObjectSetter<Data, Path>;
    setFields: FieldsSetter<Data, Path>;
};
type UnknownHelpers<Data extends Obj, Path extends string = string> = Omit<Helpers<Data, Path>, "setData" | "setFields"> & {
    setData: UnknownObjectSetter<Data, Path>;
    setFields: UnknownFieldsSetter<Data, Path>;
};
type Helpers<Data extends Obj, Path extends string = string> = {
...
type AddValidatorFn<Data extends Obj> = (validator: ValidationFunction<Data>, options?: ValidatorOptions) => void;
type SetupCurrentForm<Data extends Obj> = {
    form?: never;
    controls?: never;
    stage: "SETUP";
...
type Extender<Data extends Obj = Obj> = (currentForm: MountedCurrentForm<Data> | SetupCurrentForm<Data>) => ExtenderHandler<Data>;
/** `Record<string, unknown>` */
type Obj = Record<string, unknown>;
/** Possible field values. */
type FieldValue = string | string[] | boolean | number | File | File[] | null | undefined;
type FormControl = HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement;
type ValidationFunction<Data extends Obj> = (values: Data) => AssignableErrors<Data> | undefined | Promise<AssignableErrors<Data> | undefined>;
type TransformFunction<Data extends Obj> = (values: unknown) => Data;
 ...
 */
type FormConfigWithoutTransformFn<Data extends Obj> = {
    transform?: never;
 ...
 */
type FormConfigWithTransformFn<Data extends Obj> = {
 ...
 */
type FormConfig<Data extends Obj> = FormConfigWithTransformFn<Data> | FormConfigWithoutTransformFn<Data>;
type AnyObj = Record<string, any>;
type AnyArr = Array<any>;
/** The errors object may contain either a string or array or string per key. */
type Errors<Data extends AnyObj | AnyArr> = Data extends AnyArr ? Data[number] extends AnyObj ? Errors<Data[number]>[] : string[] | null : Data extends Date | File | File[] ? string[] | null : Data extends AnyObj ? {
    [key in keyof Data]: Data[key] extends AnyObj | AnyArr ? Errors<Data[key]> : string[] | null;
} : any;
type Keyed<Data extends AnyObj | AnyArr> = Data extends AnyArr ? Data[number] extends AnyObj ? (Keyed<Data[number]> & {
    key: string;
...
/** The errors object may contain either a string or array or string per key. */
type AssignableErrors<Data extends AnyObj | AnyArr> = Data extends AnyArr ? Data[number] extends AnyObj ? AssignableErrors<Data[number]>[] : string | string[] | null | undefined : Data extends Date | File | File[] ? string | string[] | null | undefined : Data extends AnyObj ? {
...
type Touched<Data extends AnyObj | AnyArr> = Data extends AnyArr ? Data[number] extends AnyObj ? Touched<Data[number]>[] : boolean : Data extends Date | File | File[] ? boolean : Data extends AnyObj ? {
    [key in keyof Data]: Data[key] extends AnyObj | AnyArr ? Touched<Data[key]> : boolean;
} : any;
type FormAction = (node: HTMLFormElement) => {
    destroy: () => void;
};
type TransWritable<Data extends Obj> = {
    subscribe(subscriber: (values: Keyed<Data>) => any): () => void;
    set(value: unknown): void;
    update(updater: (value: Data) => unknown): void;
};
type UnknownStores<Data extends Obj, StoreExt = Record<string, any>> = Omit<Stores<Data, StoreExt>, "data"> & {
    data: TransWritable<Data> & StoreExt;
};
type KnownStores<Data extends Obj, StoreExt = Record<string, any>> = Omit<Stores<Data, StoreExt>, "data"> & {
    data: KeyedWritable<Data> & StoreExt;
};
type PartialWritableErrors<Data extends Obj> = {
...
type KeyedWritable<Data extends Obj> = Omit<Writable<Data>, "subscribe"> & {
    subscribe(subscriber: (values: Keyed<Data>) => any): () => void;
};
/** The stores that `createForm` creates. */
type Stores<Data extends Obj, StoreExt = Record<string, any>> = {
...
type StoreFactory<Ext = Record<string, any>> = <Value>(initialValue: Value) => Writable<Value> & Ext;
type Prev = [
    never,
    0,
    1,
...
type Traverse<T extends Record<string, any> | Array<any>, Path extends string> = TraverseImpl<T, Split<Path>>;
/** @ignore */
 ...
 * @category Helper
 */
declare function getPath(el: HTMLElement | FormControl, name?: string | undefined): string;
/**
 * @category Helper
 */
declare function shouldIgnore(el: HTMLElement): boolean;
